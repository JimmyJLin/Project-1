...
// global variables
  ...
  // capture the random math value between 1-6
  // append the random math value to current position = next position = array position
  // function capture the X & Y strings from the boardPos array
  // pass the x & y value to css function to move the icon through the various positions until the final position
  function dogTwoMovingXandY(){
    for (var i = 0; i < [dogTwoMove]; i++){
  ...
  function switchPlayer(){
    if (turn == false){
      ...
      dogTwoCurrentStep += diceRoll // next position
      dogTwoMove = dogTwoCurrentStep.toString();
      dogTwoMovingXandY()
      ...
      return;
        ...
        dogOneMove = dogOneCurrentStep.toString();
        dogOneMovingXandY()
